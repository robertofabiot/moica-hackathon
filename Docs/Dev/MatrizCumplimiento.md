...
| Edición | `ServiciosPropios.test.tsx` | Sí | | En el #27 `/prestador/servicios/:id` conservaba el formulario plano: nombre, descripción, categoría, subcategoría, precio y «Guardar cambios». El #36 lo sustituye; ver «Gestión de servicios propios» |
...
servicio. Un precio nulo sigue siendo «A convenir» en presentación. La edición de un servicio ya publicado quedó en un solo formulario hasta el #36; ver «Gestión de servicios propios».
...
- **Interfaz**: 9 capturas de página completa a **375x812, 768x1024 y 1280x800**
  ...
  horizontales**. Quedaron adjuntas al [PR #33](https://github.com/robertofabiot/moica-hackathon/pull/33).
  Las seis del expediente son las capturas correctivas descritas abajo; las tres
  de bandeja se conservan como antecedente. No existe `medidas.json` de este
  incremento. No se versionan en el árbol.
...
- **Capturas actualizadas**: seis expedientes reales (`/admin/casos/6` cerrado y
  ...
  reorganizó la configuración. Las seis del expediente y las tres de la bandeja
  quedaron adjuntas al [PR #33](https://github.com/robertofabiot/moica-hackathon/pull/33),
  agrupadas por escenario a 375x812, 768x1024 y 1280x800. No se versionan en el
  árbol.
...
Es la sección a la que remiten las filas 1, 3, 4, 5 y 7 para este incremento.

> La tabla de criterios no tiene fila 6 —«Validación de entradas y manejo de
> errores» del apartado 9 del plan—, y el hueco es anterior a este incremento.
> P10B aporta evidencia que le correspondería (DTO con Bean Validation, los
> diecisiete códigos de error nuevos y sus pruebas negativas), pero completar esa
> fila exigiría reconstruir su historia desde P1 y queda fuera de este PR.
...
Controles del rediseño de `/prestador/servicios` (listado) y
`/prestador/servicios/:id` (edición: datos, estado e imágenes) según el sistema
de diseño y el asistente de publicación, con el resultado real de cada
comprobación.

- **Local**: `npx prettier --check .`, `npx eslint .`, `npx tsc -b --noEmit` y
  `npx vitest run --maxWorkers=1` en el frontend sobre `bc19963`: 351 pruebas
  en 40 archivos en verde. `ServiciosPropios.test.tsx` cubre 4 casos (vacío,
  switch de publicación, edición de subcategoría y subida de imagen).
  `NuevoServicio.test.tsx` conserva 8. `Boton.test.tsx` suma la variante
  fantasma. Recorrido en el navegador contra la API local: vacío de
  `/prestador/servicios` y error con migas en `/prestador/servicios/1`.
  `docker compose --env-file .env.example config -q` en verde. `./mvnw verify`
  no se ejecutó aquí: este PR no toca el backend; el check de CI lo cubre.
- **CI**: el #36 se abre contra `develop`. El merge-base con `origin/develop`
  es `3d0f07d`, el HEAD actual de esa rama. Los enlaces de CI se anotan cuando
  existan; no se describen de memoria.
- **Rama**: `feature/ui-gestion-servicios-propios` nace de `develop` actualizado.
  El #36 se abre contra esa rama, no contra `main`.

Una casilla vacía significa que ahí no aplica, no que fallara.

| Control | Cómo se comprueba | Local | CI | Evidencia |
|---|---|---|---|---|
| Layout del listado | Revisión de `ServiciosPropios.tsx`, `MarcoDeGestionDeServicios.tsx` y `servicios.module.css` | Sí | | `BarraLateral` (ítem `inicio`; destinos literales a `/panel`, `/mensajes`, `/prestador` y `/seguridad` para no cerrar un ciclo con el panel). Fondo `--color-neutral-50`. Contenido `max-width: 64rem`. Visible desde `48rem`; en teléfono `display: none`. Título «Mis servicios» y CTA «+ Publicar nuevo servicio» a `/prestador/servicios/nuevo` |
| Resumen | `ServiciosPropios.test.tsx` y revisión | Sí | | Chips de total publicados, activos (punto `--color-secondary-500`) e inactivos (punto `--color-neutral-400`) |
| Tarjetas | `ServiciosPropios.test.tsx` | Sí | | Grid `repeat(auto-fill, minmax(min(100%, 320px), 1fr))`. Miniatura 16:9 o `IconoDeCategoria`. Píldoras `ACTIVO` (`--color-secondary-50` / `--color-secondary-500`, punto pulsante) e `INACTIVO` (`--color-neutral-100` / `--color-neutral-600`). Categoría y subcategoría. Precio `C$ 350.00` o «A convenir». Acción secundaria «Editar» con `IconoLapiz` |
| Switch de publicación | `ServiciosPropios.test.tsx` | Sí | | `role="switch"` con `aria-label` «Publicación de …». Conserva `PUT /api/prestador/servicios/{id}/estado`. Indicador `isPending` |
| Estado vacío | `ServiciosPropios.test.tsx` | Sí | | Si `lista.length === 0`: maletín, «Aún no has publicado ningún servicio» y el mismo CTA primario |
| Edición: migas y cabecera | `ServiciosPropios.test.tsx` | Sí | | Migas «Mis servicios» > «Editar servicio». Nombre actual, píldoras de categoría y «Volver al listado» |
| Datos generales | `ServiciosPropios.test.tsx` | Sí | | `Entrada` para nombre y precio (prefijo `C$`). Categoría y subcategoría encadenadas; el catálogo tardío no pierde la subcategoría. «Guardar cambios generales» a la derecha. Conserva `PUT` de datos |
| Estado de la publicación | Revisión de `EstadoDelServicio.tsx` | Sí | | Tarjeta que explica activo (visible si el perfil está verificado) e inactivo (oculto, no se elimina). El mismo switch que el listado. Conserva `useCambioDeEstadoDeServicio` |
| Galería | `ServiciosPropios.test.tsx` | Sí | | Grid de fotos, insignia «Foto principal» en la primera, dropzone (JPG, PNG o WebP, máx. 5 MB), confirmación accesible al eliminar. Conserva subida, orden, texto alternativo y borrado |
| Variante fantasma | `Boton.test.tsx` | Sí | | `variante="fantasma"` en `Boton`. Se usa en acciones secundarias de la galería |
| Ciclo de importación | Recorrido en el navegador | Sí | | `MarcoDeGestionDeServicios` no importa `RUTA_PANEL` del panel: un import inverso dejaba `RUTA_PANEL` en zona muerta y `#root` vacío. Tras el arreglo, `/prestador/servicios` monta |
| Pruebas del frontend | `npx vitest run --maxWorkers=1` | Sí | | 351 en verde, 40 archivos. `ServiciosPropios.test.tsx` 4 casos; `NuevoServicio.test.tsx` 8 |
| Nace de `develop` | `git merge-base origin/develop HEAD` | Sí | | Devuelve `3d0f07d`, el HEAD de `develop`. El #36 se abre contra esa rama, no contra `main` |
| Capturas a tres tamaños | Chrome a 375x812, 768x1024 y 1280x800 | Sí | | Adjuntas al Pull Request: teléfono, tableta y escritorio |
| Medición de desbordamiento | `scrollWidth` frente a `clientWidth` | **No** | | **No registrada.** Las capturas se adjuntaron; no se anotó `scrollWidth == clientWidth` |

El contrato de `GET`/`PUT`/`POST /api/prestador/servicios` y de las imágenes no
cambia. Un precio nulo sigue siendo «A convenir» en presentación. Activar sigue
exigiendo cuenta operativa, prestador disponible y al menos verificación básica.

## Flujo visual de solicitudes de servicio

Controles del rediseño de `/solicitudes` (bandejas), `/solicitudes/:id` (detalle,
acciones, historial, chat, contactos, calificación y reporte) y
`/explorar/servicios/:id/solicitar` (formulario de contratación) según el sistema
de diseño, con el resultado real de cada comprobación.

- **Local**: `npx prettier --check` sobre los archivos tocados, `npx eslint` sobre
  `src/capacidades/solicitud` y `src/capacidades/prestador/api.ts`, `npx tsc -b --noEmit`
  y `npx vitest run --maxWorkers=1` en el frontend sobre `5b18211`: 352 pruebas
  en 40 archivos en verde. `MisSolicitudes.test.tsx` cubre 6 casos. Recorrido en el
  navegador contra la API local: vacío de `/solicitudes` a 375 px (`scrollWidth ==
  clientWidth`) y a dos columnas en escritorio; formulario de contratación con
  validación visible. `./mvnw verify` no se ejecutó aquí: este PR no toca el
  backend; el check de CI lo cubre.
- **CI**: el #37 se abre contra `develop`. El merge-base con `origin/develop`
  es `9932cba`, el HEAD actual de esa rama. Los enlaces de CI se anotan cuando
  existan; no se describen de memoria.
- **Rama**: `feature/ui-flujo-solicitudes-servicio` nace de `develop` actualizado.
  El #37 se abre contra esa rama, no contra `main`.

Una casilla vacía significa que ahí no aplica, no que fallara.

| Control | Cómo se comprueba | Local | CI | Evidencia |
|---|---|---|---|---|
| Marco y barra lateral | Revisión de `MarcoDeSolicitudes.tsx` y `solicitudes.module.css` | Sí | | `BarraLateral` con destinos literales a `/panel`, `/mensajes`, `/prestador` y `/seguridad` para no cerrar un ciclo. Oculta en teléfono (`display: none`); visible desde `64rem` con el mismo desplazamiento que el resto de pantallas autenticadas |
| Bandejas de prestador | `MisSolicitudes.test.tsx` | Sí | | Chips «Enviadas: X» y «Recibidas: Y» con desplazamiento a `#enviadas` y `#recibidas`. Ambas bandejas montadas a la vez, sin tabs. Vacío literal «Todavía no has enviado solicitudes.» y «Todavía no has recibido solicitudes.». En escritorio, `grid-template-columns: 1fr 1fr` |
| Bandejas de cliente | `MisSolicitudes.test.tsx` | Sí | | Sin perfil de prestador ni recibidas, no se pinta la bandeja de recibidas. Invitación a explorar y a crear perfil en `/prestador` |
| Tarjetas y píldoras | Revisión y `MisSolicitudes.test.tsx` | Sí | | Enlace a `/solicitudes/:id`. Nombre del servicio, contraparte, municipio y fecha. Píldoras `PENDIENTE` (ámbar), `ACEPTADA` (teal), `COMPLETADA` (verde) y `RECHAZADA`/`CANCELADA` (rojo tenue) |
| Detalle | `DetalleDeSolicitud.test.tsx` | Sí | | Cabecera con «Volver a mis solicitudes», título y píldora. Una columna en teléfono (necesidad, acciones, contactos, chat, historial, calificación y reporte). Desde `64rem`, columna principal 60 % y lateral 40 % (acciones e historial). Chat, calificación y reporte no se reescriben |
| Acciones e historial | `DetalleDeSolicitud.test.tsx` | Sí | | Textos exactos: «Aceptar», «Sí, aceptar», «Rechazar», «Sí, rechazar», «Cancelar solicitud», «Cancelar con motivo», «Motivo», «Confirmar cancelación» y «Marcar como completada». Timeline con nodos circulares y `aria-label="Historial"` |
| Contactos | `ContactosDelPrestador.test.tsx` | Sí | | Lista `aria-label="Medios de contacto del prestador"` en texto, sin `<a>`. Icono decorativo |
| Formulario de contratación | `NuevaSolicitud.test.tsx` | Sí | | Etiquetas «Qué necesitas», «Municipio», «Dirección, sector o referencia» y «Fecha preferida (opcional)». Botón «Enviar solicitud» y estado «Enviando solicitud…». Tarjeta a ancho completo en teléfono y `max-width: 40rem` desde `48rem` |
| 404 del perfil | Revisión de `prestador/api.ts` y `tsc` | Sí | | `obtenerPerfil` trata `PERFIL_NO_ENCONTRADO` y `ErrorDeApi.estado === 404` como «aún no hay perfil». `error.status` no existe en el tipo y rompía `tsc` |
| Pruebas del frontend | `npx vitest run --maxWorkers=1` | Sí | | 352 en verde, 40 archivos. `MisSolicitudes.test.tsx` 6 casos |
| Nace de `develop` | `git merge-base origin/develop HEAD` | Sí | | Devuelve `9932cba`, el HEAD de `develop`. El #37 se abre contra esa rama, no contra `main` |
| Capturas a tres tamaños | Chrome a 375x812, 768x1024 y 1280x800 | Sí | | Adjuntas al Pull Request: teléfono, tableta y escritorio |
| Medición de desbordamiento | `scrollWidth` frente a `clientWidth` | **No** | | **No registrada** en las quince combinaciones. En el recorrido local, `/solicitudes` a 375 px cumplió `scrollWidth == clientWidth` |

El contrato de `GET`/`POST` de solicitudes, chat, contactos, calificación y
reporte no cambia. Aceptar, rechazar, cancelar y completar siguen autorizándose
en el backend. Los contactos siguen ocultos hasta la aceptación y no se pintan
como enlaces.

## Panel administrativo, cola de verificaciones y segundo factor

Controles del rediseño de `/admin` (panel), `/admin/verificaciones` (cola y
expediente) y `/verificar-segundo-factor` (TOTP de sesión) según el sistema de
diseño, con el resultado real de cada comprobación.

- **Local**: `npx prettier --check` sobre los archivos tocados, `npx eslint`
  sobre las capacidades `admin`, `verificacion` y `auth` afectadas, `npx tsc -b
  --noEmit` y `npx vitest run` en el frontend sobre `43a0454`: 352 pruebas en 40
  archivos en verde. `PanelAdministrativo.test.tsx` conserva 10 casos,
  `ColaDeVerificaciones.test.tsx` 13 y `VerificacionSegundoFactor.test.tsx` 10.
  `./mvnw verify` no se ejecutó aquí: este PR no toca el backend; el check de CI
  lo cubre.
- **CI**: el #38 se abre contra `develop`. El merge-base con `origin/develop`
  es `77d7c60`, el HEAD actual de esa rama. Los enlaces de CI se anotan cuando
  existan; no se describen de memoria.
- **Rama**: `feature/ui-admin-verificaciones-2fa` nace de `develop` actualizado.
  El #38 se abre contra esa rama, no contra `main`.

Una casilla vacía significa que ahí no aplica, no que fallara.

| Control | Cómo se comprueba | Local | CI | Evidencia |
|---|---|---|---|---|
| Panel administrativo | `PanelAdministrativo.test.tsx` | Sí | | `h1` «Área administrativa». Badge «Área de Administración». Credencial con iniciales, nombre, correo y fecha de asignación. Módulos táctiles a `/admin/verificaciones` (enlace exacto «Verificaciones documentales») y `/admin/casos` («Casos de moderación»), ambos funcionales, sin badge de «Próximamente». Pie «Volver al inicio» |
| Cola de verificaciones | `ColaDeVerificaciones.test.tsx` | Sí | | `h1` «Verificaciones documentales». Filtros «Estado» y «Nivel» a `min-height: 3rem` y `font-size: 1rem`. Tabla con scroll táctil. Nombre público en negrita, píldora de nivel y de estado. Botones «Abrir» / «Cerrar» con `aria-label` exacto |
| Expediente y resolución | `ColaDeVerificaciones.test.tsx` | Sí | | Visor de adjuntos con `target="_blank"` y `rel="noopener noreferrer"`. Textos de acción intactos: «Tomar para revisar», «Aprobar», «Rechazar con motivo», «Rechazar», «Revocar con motivo», «Sí, revocar la verificación». Motivo con `label` «Motivo» y casilla de confirmación |
| Segundo factor | `VerificacionSegundoFactor.test.tsx` | Sí | | `h1` «Verifica tu segundo factor», `label` «Código de verificación», submit «Verificar y entrar», salida «Salir de la sesión». `inputMode="numeric"`, `autoComplete="one-time-code"`, `pattern="[0-9]*"`. `role="alert"` en errores. Misma tarjeta que el inicio de sesión |
| Viewport PWA | Revisión de `index.html` | Sí | | `viewport-fit=cover` para que `env(safe-area-inset-*)` aplique en PWA instalada |
| Pruebas del frontend | `npx vitest run` | Sí | | 352 en verde, 40 archivos. No se suman casos |
| Nace de `develop` | `git merge-base origin/develop HEAD` | Sí | | Devuelve `77d7c60`, el HEAD de `develop`. El #38 se abre contra esa rama, no contra `main` |
| Capturas a tres tamaños | Chrome a 375x812, 768x1024 y 1280x800 | Sí | | Adjuntas al Pull Request: teléfono, tableta y escritorio |
| Medición de desbordamiento | `scrollWidth` frente a `clientWidth` | **No** | | **No registrada.** Las capturas se adjuntaron; no se anotó `scrollWidth == clientWidth` |

El contrato de `GET /api/admin/resumen`, de la cola y las mutaciones de
verificación, y de `POST /api/auth/sesion/segundo-factor` no cambia. El área
sigue exigiendo rol administrativo y segundo factor verificado en esa sesión.

## Pantalla de acceso no autorizado

Controles de la pantalla independiente de HTTP 401 / 403 (`AccesoNoAutorizado`)
que sustituye el acceso denegado plano de `RutaAdministrativa`, con el resultado
real de cada comprobación.

- **Local**: Prettier sobre los archivos tocados, ESLint sobre esos mismos
  archivos, `tsc -b --noEmit` y `npx vitest run` en el frontend sobre `272bba8`:
  **398 pruebas en 45 archivos** en verde. `AccesoNoAutorizado.test.tsx` cubre 6 casos.
  `PanelAdministrativo.test.tsx` conserva 10 y ahora afirma los títulos nuevos.
  Recorrido en el navegador: 401, 403 por permisos y 403 por 2FA; CTAs a
  `/iniciar-sesion`, `/seguridad`, `/explorar` y `/`; botones en columna a
  390 px y en fila en escritorio. `./mvnw verify` no se ejecutó aquí: el #41
  no toca el backend; el check de CI lo cubre.
- **CI**: el #41 se abre contra `develop`. El merge-base con `origin/develop`
  es `1e53383`, el HEAD actual de esa rama. Los enlaces de CI se anotan cuando
  existan; no se describen de memoria.
- **Rama**: `feature/ui-pantalla-acceso-no-autorizado` nace de `develop`
  actualizado. El #41 se abre contra esa rama, no contra `main`.

Una casilla vacía significa que ahí no aplica, no que fallara.

| Control | Cómo se comprueba | Local | CI | Evidencia |
|---|---|---|---|---|
| 401 sesión inactiva | `AccesoNoAutorizado.test.tsx` | Sí | | `h1` «Tu sesión no está activa». Badge «HTTP 401 · SESIÓN REQUERIDA». Primario «Iniciar sesión» a `/iniciar-sesion`. También infiere el caso si solo llega `codigo={401}` |
| 403 permisos | `AccesoNoAutorizado.test.tsx` y `PanelAdministrativo.test.tsx` | Sí | | `h1` «Esta zona requiere otros permisos». Badge «HTTP 403 · ACCESO RESTRINGIDO». Primario «Volver al inicio» a `/`, o a `destinoRetorno`. `RutaAdministrativa` lo muestra a una cuenta ordinaria |
| 403 segundo factor | `AccesoNoAutorizado.test.tsx` y `PanelAdministrativo.test.tsx` | Sí | | `h1` «Verificación adicional requerida». Primario «Configurar seguridad» a `/seguridad`. `RutaAdministrativa` lo muestra a un administrador sin el segundo factor verificado en esa sesión |
| CTA secundaria y marca | `AccesoNoAutorizado.test.tsx`; recorrido en el navegador | Sí | | «Volver a explorar» a `/explorar`. El lockup horizontal enlaza a `/` con `aria-label="Moica, ir al inicio"` |
| Mensaje personalizado | `AccesoNoAutorizado.test.tsx` | Sí | | `mensajePersonalizado` sustituye la explicación y sigue en `role="alert"` |
| Ilustración | Revisión de `IlustracionDeAccesoDenegado.tsx` | Sí | | SVG `viewBox="0 0 320 220"` con `aria-hidden="true"`: sombra, halo, escudo teal, candado naranja, senderos punteados y destellos en cruz. Solo tokens de `global.css` |
| Layout y a11y | Revisión de `AccesoNoAutorizado.module.css` y recorrido | Sí | | Standalone, sin `BarraLateral`. `min-height: 100dvh`, `env(safe-area-inset-*)`, botones en columna a ancho completo y en fila desde `48rem`. Entrada suave de 300 ms y `prefers-reduced-motion: reduce`. El foco visible nativo de Moica no se apaga |
| Pruebas del frontend | `npx vitest run` | Sí | | 398 en verde, 45 archivos: las 392 de `develop` más 6 del #41 |
| Nace de `develop` | `git merge-base origin/develop HEAD` | Sí | | Devuelve `1e53383`, el HEAD de `develop`. El #41 se abre contra esa rama, no contra `main` |
| Capturas a tres tamaños | Chrome a 375x812, 768x1024 y 1280x800 | Sí | | Adjuntas al Pull Request: teléfono, tableta y escritorio |
| Medición de desbordamiento | `scrollWidth` frente a `clientWidth` | **No** | | **No registrada.** Las capturas se adjuntaron; no se anotó `scrollWidth == clientWidth` |

Esta pantalla **no** es un control de seguridad: quien pida `/api/admin/...`
sin rol o sin el segundo factor verificado sigue recibiendo 403 del backend.
...
- **Auditoría de vistas pendientes:** se inspeccionaron todas las rutas y páginas (`auth`, `busqueda`, `panel`, `prestador`, `servicio`, `solicitud`, `admin`, `moderacion`, `verificacion`). No existen pantallas placeholder temporales ni wireframes mínimos desatendidos: todas las 23 rutas cuentan con implementación completa, integración de API real y pruebas automatizadas.
- **Estandarización de navegación (Desktop y Mobile):**
  - Miga de pan en `/admin/casos/:idCaso` y `/admin/medidas` unificada con `IconoChevronIzquierda` y clase `.enlaceVolver`.
  - Enlaces de regreso al área administrativa (`/admin/casos`, `/admin/casos/:idCaso`, `/admin/verificaciones`) y al inicio unificados con `.enlaceDePie` y min-height de 2.75rem (44 px).
- **Accesibilidad y objetivos táctiles Mobile-First (WCAG AAA):**
  - Barra lateral de navegación (`BarraLateral.module.css`): `.control` cuenta con `min-width: 2.75rem` y `min-height: 2.75rem` (44x44 px) explícitos en todos los viewports.
  - Botones secundarios y de acción compacta (`secciones.module.css`): `.botonPequeno` cuenta con `min-height: 2.75rem` (44 px) e `inline-flex` centrado para toques táctiles precisos en dispositivos móviles.
- **Validación automatizada:**
  - Vitest: 45 archivos de prueba pasando, 403 pruebas en total (100% verde).
...
| Integración PostgreSQL | `DemoIT` y `ArranqueDeDemoIT`: creación, repetición, relaciones, visibilidad, filtros, recuperación parcial, preservación ajena, catálogo con IDs nuevos, rollback y concurrencia; GET anónimos comprueban listado, detalles y perfiles por la API real; ejecutadas por el job backend del PR con Testcontainers |
