...
% LÍMITE DEL SISTEMA
% ====================
\draw[system] (1.5, -24.5) rectangle (24.5, 28.5);
\node[below right, font=\Large\bfseries\sffamily, text=black!70] at (2.5, 28) {MOICA - MVP};
...
\node[usecase] (UC_VerIns) at (7, 26.5) {Ver insignia de\\verificación del perfil};
\node[usecase] (UC_Exp) at (7, 24) {Explorar perfiles\\y servicios};
\node[usecase] (UC_Con) at (7, 21.5) {Consultar portafolio\\y reputación};

% --- Columna de la cuenta (acceso y credenciales) ---
\node[usecase] (UC_CamCla) at (13, 24) {Cambiar contraseña};
\node[usecase] (UC_Cerrar) at (13, 21.5) {Cerrar sesión};
...
\node[usecase] (UC_SolV) at (7, -19) {Solicitar verificación\\(Básica / Profesional)};
\node[usecase] (UC_EstV) at (7, -21.5) {Consultar estado de\\su verificación};
...
\node[usecase] (UC_RevV) at (19, -4) {Revisar solicitudes\\de verificación};
\node[usecase] (UC_ResV) at (19, -6.5) {Aprobar, rechazar o\\revocar verificación};
...
\draw[assoc] (C) -- (UC_Rep);
% Cuenta: ruta ortogonal por el hueco que queda entre «Explorar perfiles» y
% «Consultar portafolio», para no atravesar ningún caso de uso.
\coordinate (Cuenta1) at (3.2, 22.75);
\coordinate (Cuenta2) at (10, 22.75);
\draw[assoc, rounded corners=10pt] (C) -- (3.2, 11) -- (Cuenta1) -- (Cuenta2) -- (10, 24) -- (UC_CamCla.west);
\draw[assoc, rounded corners=10pt] (Cuenta2) -- (10, 21.5) -- (UC_Cerrar.west);
...
% Verificación del prestador: se baja por el lateral para no cruzar los casos de uso
\draw[assoc, rounded corners=15pt] (P) |- (UC_SolV.west);
\draw[assoc, rounded corners=15pt] (P) |- (UC_EstV.west);
% Rutada ortogonalmente por debajo para no cruzar los casos de uso
\draw[assoc, rounded corners=15pt] (P) -- (-2, -23.2) -| (UC_CalC.south);
...
\draw[assoc] (A) -- (UC_RevV);
\draw[assoc] (A) -- (UC_ResV);
...
% UC_RevV extends UC_ResV (Línea recta vertical)
\draw[extend] (UC_RevV.south) -- node[pos=0.5, fill=gray!3, font=\scriptsize\sffamily, text=purple!70!black, inner sep=2pt] {$\ll$\textsf{extends}$\gg$} (UC_ResV.north);
