...
- **Local**: ejecutado en la máquina de desarrollo (Windows 11, Docker Desktop,
  Node 22, JDK compilando con `release 21`), con PostgreSQL publicado en
  `localhost:5433`. El último commit de código incluye la corrección de la
  subcategoría al editar. Este cambio documental viaja en el mismo PR.
- **CI**: este PR se abre contra `develop`. Los enlaces de CI se anotan cuando
  existan; no se describen de memoria.
- **Rama**: `feature/servicios-busqueda-publica` nace de `develop` en
  `f195360`. No incorpora la rama de trabajo del frontend definitivo.
...
| Descubrimiento público | `DescubrimientoIT` y `CatalogoTerritorialIT` | Sí | | `GET` de catálogos, listado y detalle sin sesión. Solo `ACTIVO` de cuenta `ACTIVA`, prestador `DISPONIBLE` y perfil con al menos básica. Texto, categoría, subcategoría y municipio se combinan. Orden `nombre`, `id`. Sin contactos, correos, documentos ni claves. `precioReferencia` viaja nulo |
| Frontend de gestión y exploración | `npm run test` | Sí | | 167 pruebas en 20 archivos: 6 en `ExplorarServicios.test.tsx` (carga, error, vacío, filtros, «A convenir», insignia) y 6 en `ServiciosPropios.test.tsx` (listado, alta, estado, imágenes y subcategoría al editar) más 1 de rutas en `App.test.tsx` |
| Verificación local completa | Comandos del criterio de salida | Sí | | Backend: `./mvnw -B -ntp verify` — 138 unitarias, 307 de integración, Spotless limpio, SpotBugs 0. Frontend: `format:check`, `lint`, `typecheck`, `test` y `build` en verde sobre el código de P5; la prueba nueva de subcategoría al editar se volvió a ejecutar en verde. Raíz: `git diff --check` sin salida y `docker compose config` válido |
| Capturas a tres tamaños | Chrome a 375x812, 768x1024 y 1280x800 | Sí | | Recorrido de exploración, vacío, detalle, insignia, perfil público y administración propia. `scrollWidth == clientWidth` en las 15. Carpeta fuera del repositorio; se adjunta al PR |
| Bucket público real de una imagen de servicio | Pasos de `Almacenamiento.md` contra R2 | | | Solo se anota si las credenciales locales están disponibles y el recorrido se ejecutó. No se inventa evidencia |
