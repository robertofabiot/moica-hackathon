...
- **Local**: Prettier sobre los archivos tocados, ESLint sobre esos mismos
  ...
  **398 pruebas en 45 archivos** en verde. `AccesoNoAutorizado.test.tsx` cubre 6 casos.
  ...
  390 px y en fila en escritorio. `./mvnw verify` no se ejecutó aquí: este PR
  no toca el backend; el check de CI lo cubre.
- **CI**: este PR se abre contra `develop`. El merge-base con `origin/develop`
...
- **Rama**: `feature/ui-pantalla-acceso-no-autorizado` nace de `develop`
  actualizado. Este PR se abre contra esa rama, no contra `main`.
...
| Pruebas del frontend | `npx vitest run` | Sí | | 398 en verde, 45 archivos: las 392 de `develop` más 6 de este PR |
| Nace de `develop` | `git merge-base origin/develop HEAD` | Sí | | Devuelve `1e53383`, el HEAD de `develop`. Este PR se abre contra esa rama, no contra `main` |
| Capturas a tres tamaños | Chrome a 375x812, 768x1024 y 1280x800 | Sí | | Adjuntas al Pull Request: teléfono, tableta y escritorio |
| Medición de desbordamiento | `scrollWidth` frente a `clientWidth` | **No** | | **No registrada.** Las capturas se adjuntaron; no se anotó `scrollWidth == clientWidth` |
