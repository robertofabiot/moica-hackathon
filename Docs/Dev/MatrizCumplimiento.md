...
- **Local**: ejecutado en la máquina de desarrollo (Windows 11, Docker Desktop,
  ...
  para las pruebas y el Compose local —publicado en `localhost:5433`— para el
  recorrido integrado.
- **Validación local con Docker activo** (1 de septiembre de 2026, sobre
  `ed7b0d3`). Docker Engine 29.7.2, Docker Desktop. En `backend`,
  ...
  `BUILD SUCCESS` en 7:57 min. Testcontainers 1.21.4 levantó de verdad
  `postgres:15-alpine`; **no se usa H2 en ninguna prueba**. Los cuatro
  conjuntos de P9 corrieron —`EsquemaDeCasosDeModeracionIT` 35,
  `ReporteDeParticipanteIT` 28, `ConcurrenciaDeReporteIT` 3 y
  `ReporteAPresentarTest` 7—. En `frontend`, `format:check`, `lint`,
  `typecheck`, `test` (**282 en 32 archivos**) y `build` en verde. En la raíz,
...
- **Rama**: `feature/casos-moderacion` nace de `develop` en `763a4ba`, el merge
  de P8, comprobado con `git merge-base origin/develop HEAD`.
...
| El caso propio es propio | `ReporteDeParticipanteIT.nadieVeElCasoQuePresentoLaContraparte` y `despuesDeReportarDevuelveElCasoPropioYCierraLaAccion` | Sí | Sí | Tras reportar el cliente, la consulta del prestador devuelve `casoAbierto: null`, `puedeReportar: true` y su cuerpo no contiene el identificador del caso ajeno. |
...
| Frontend | `npm run test` | Sí | Sí | 282 pruebas en 32 archivos: las 265 anteriores más 17 nuevas en `ReporteDeSolicitud.test.tsx` —visibilidad según el historial de la solicitud, etiquetas y nombres accesibles, validación con `aria-invalid` y `aria-describedby`, contador de caracteres, cuerpo enviado, resumen posterior, error del backend con lo escrito conservado y reintento, prevención del doble envío y cuenta restringida—. |
| Sin regresiones de P6, P7 y P8 | `DetalleDeSolicitud.test.tsx`, `ChatDeSolicitud.test.tsx`, `ContactosDelPrestador.test.tsx` y `CalificacionDeSolicitud.test.tsx` | Sí | Sí | Las pruebas anteriores siguen verdes sin debilitarse: solo se les añadió la preparación de la ruta nueva que el detalle monta ahora. |
| Verificación local completa | Comandos del criterio de salida | Sí | Sí | Backend: `./mvnw -B -ntp verify` — **167 unitarias y 488 de integración**, 0 fallos, 0 errores y 0 omitidas, Spotless limpio, SpotBugs sin hallazgos, `BUILD SUCCESS`. Frontend: `format:check`, `lint`, `typecheck`, `test` (282) y `build`, todos en verde. Raíz: `git diff --check` sin hallazgos y `docker compose --env-file .env.example config -q` válido. |
...
**Pendiente preexistente, ajeno a P9**: el contenedor `moica_pgadmin` sigue
