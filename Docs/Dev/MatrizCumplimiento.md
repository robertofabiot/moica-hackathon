...
| Despliegue en Railway | Panel del proveedor | **No** | | **Pendiente.** Requiere que el propietario inicie sesión. Sin proyecto, servicios, URL pública, métricas ni smoke público |
| Integración con R2 | Carga real contra los dos buckets | **No** | | **Pendiente.** Sin credenciales reales, el backend arranca y esas superficies responden 503; eso no demuestra la integración |
...
El smoke se ejecuta sobre HTTP de loopback con un proxy que declara la
terminación HTTPS, así que **no** demuestra TLS público ni el comportamiento de
las cookies en un navegador real: esos pasos siguen siendo obligatorios contra
la URL de Railway y están enumerados en
[DespliegueProduccion.md](DespliegueProduccion.md).
