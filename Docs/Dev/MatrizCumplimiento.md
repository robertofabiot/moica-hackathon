...
- **CI**: este PR se abre contra `develop`. El merge-base con `origin/develop`
  ...
  existan; no se describen de memoria.
- **Rama**: `feature/ui-explorar-servicios` nace de `develop` actualizado. Este
...
| Nace de `develop` | `git merge-base origin/develop HEAD` | Sí | | Devuelve `4b512fc`, el HEAD de `develop`. Este PR se abre contra esa rama, no contra `main` |
| Capturas a tres tamaños | Chrome a 375x812, 768x1024 y 1280x800 | **No** | | **Pendiente.** El espacio para teléfono, tableta y escritorio queda en el Pull Request; no hay todavía el juego de capturas ni `scrollWidth == clientWidth` |
| Medición de desbordamiento | `scrollWidth` frente a `clientWidth` | **No** | | **No registrada.** Sin capturas no se anota desbordamiento |

El contrato de `GET /api/servicios` no cambia. No se inventan reputaciones: la
nota de las tarjetas es maqueta visual hasta P8. El detalle público y el perfil
del prestador conservan su layout anterior.
